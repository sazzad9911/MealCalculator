                ...
                db.collection("mess").doc(key).collection('event').orderBy("date", "asc").limit(10)
                    ...
                    .then(function(querySnapshot) {
                        querySnapshot.forEach(function(doc) {
                            ...
                            var date = doc.get('date').toDate();
                            var d = date.getDate() + '-' + (date.getMonth() + 1) + '-' + date.getFullYear();
                            document.getElementById('event-st').insertAdjacentHTML('afterend', "<div class='event-box'><h4>" + doc.get('title') + "</h4><p>" + doc.get('text') + "</p><h6>" + d + "</h6><button value='" + j + "' onclick='ViewTicket(" + doc.get('cost') + ")'>View Ticket</button><button value='" + doc.id + "' id='" + j + "' onclick='BuyTicket(" + doc.get('cost') + "," + j + "," + code + ")'>Buy Ticket</button><label class='switch' id='100'><input type='checkbox'><span class='slider round'></span></label></div>");
